...
class MainPanel:
    ...
    def __init__(self, username, send_message, send_image, close_callback):
        print("初始化主界面")
    ...
    def show(self):
        ...
        Button(main_frame, text="发送", bg="blue", font=("宋体", 14), fg="white", command=self.send_message) \
            .grid(row=3, column=1, pady=5, padx=10, sticky=W, ipady=3, ipadx=10)
        Button(main_frame, text="发送图片", bg="orange", font=("宋体", 14), fg="white", command=self.send_image) \
        ...
        Button(main_frame, text="清空", bg="red", font=("宋体", 14), fg="white", command=self.clear_send_text) \
            .grid(row=3, column=1, pady=5, sticky=W, padx=(250, 0), ipady=3, ipadx=10)
    ...
    def recv_images(self, user, filepath):
        try:
            self.message_text.config(state=NORMAL)
            title = user + " " + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + "\n"
            if user == self.username:
            ...
            self.message_text.image_create(END, image=tk_image)
