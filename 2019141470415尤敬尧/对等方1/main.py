...
if __name__ == '__main__':
    ...
    while True:
        ...
        if opt0 =='get':
            ...
            option=input("请完整输入所要修改的节点的信息(ex: port_number 10815)")
            if option.split()[0]=="port_number":
                curpath = os.path.dirname(os.path.realpath(__file__))
                cfgpath = os.path.join(curpath, 'config.ini')  
                print(cfgpath)
                conf.read(cfgpath, encoding='utf-8')
                str=option.split()[1]
                conf.set("Peer-0","port_number",str) 
                conf.write(open(cfgpath, "r+", encoding="utf-8"))  # r+模式
                print("修改成功！")
                
            elif option.split()[0]=="ip_addr":
                curpath = os.path.dirname(os.path.realpath(__file__))
                cfgpath = os.path.join(curpath, 'config.ini')  
                print(cfgpath)
                conf.read(cfgpath, encoding='utf-8')
                str=option.split()[1]
                conf.set("Peer-0","ip_addr",str) 
                conf.write(open(cfgpath, "r+", encoding="utf-8"))  # r+模式
                print("修改成功！")
          
            elif option.split()[0]=="share_dir":
                curpath = os.path.dirname(os.path.realpath(__file__))
                cfgpath = os.path.join(curpath, 'config.ini')  
                print(cfgpath)
                conf.read(cfgpath, encoding='utf-8')
                str=option.split()[1]
                conf.set("Peer-0","share_dir",str) 
                conf.write(open(cfgpath, "r+", encoding="utf-8"))  # r+模式
                print("修改成功！")
